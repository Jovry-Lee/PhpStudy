...
class Worker
{
    ...
    {
        // 1. 校验运行环境，目前仅支持cli模式；2. 设置当前处理的操作系统
        static::checkSapiEnv();
        // 1. 设置错误处理句柄；2. 初始化pid文件；3. 初始化workerman日志文件及读写权限（0622）；4. 设置状态为"启动"状态；5. 设置统计信息（启动时间、文件路径）；6. 设置进程名；7. 初始化id为0；8. 初始化定时器，注册"SIGALRM"信号。
        static::init();
        // 1. 解析校验命令；2. 解析运行模式（-d/-g）; 3. 校验当前主进程与执行命令的的关系；4.通过发送信号方式与子进程通信获取相应的数据或者执行相应操作
        static::parseCommand();
        static::daemonize();
        static::initWorkers();
    ...
    protected static function monitorWorkersForLinux()
    {
        ...
        while (1) {
            ...
            // pcntl_signal_dispatch()函数不能实现真正的异步信号通知，因为TA的用法就是[ 不断地分发 ]，就是说TA并不是说分发一次就能一直收到信号，而是说上次信号响应过后如果你还想让捕捉[ 还尚未到来的 ]下次信号，那你就一定要再次dispatch！
            \pcntl_signal_dispatch();
    ...
    public function listen()
    {
        ...
        if (!$this->_mainSocket) {
            ...
            // Flag.
            // 对于UDP套接字，必须使用STREAM_SERVER_BIND作为flags参数。
            $flags = $this->transport === 'udp' ? \STREAM_SERVER_BIND : \STREAM_SERVER_BIND | \STREAM_SERVER_LISTEN;
            $errno = 0;
            $errmsg = '';
            // SO_REUSEPORT. 允许对同一个ip:端口对进行多个绑定，甚至可以从不同的进程进行绑定。
            if ($this->reusePort) {
            ...
            if ($this->transport === 'ssl') {
                // 启用或禁用流上的加密。一旦建立了加密设置，就可以通过在enable参数中传递true或false来动态地开启和关闭加密。
            ...
            if (\function_exists('socket_import_stream') && static::$_builtinTransports[$this->transport] === 'tcp') {
                ...
                // SO_KEEPALIVE：报告连接是否通过定期的消息传输保持活动状态。如果连接的套接字无法响应这些消息，连接将被断开，并使用SIGPIPE信号通知写入该套接字的进程。
                \socket_set_option($socket, \SOL_SOCKET, \SO_KEEPALIVE, 1);
                // TCP_NODELAY：报告Nagle TCP算法是否被禁用。
